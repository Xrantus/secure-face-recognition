"""Live face detection + recognition FOR 128D TESTS.

This script is a standalone copy of `main.py` that imports and uses
the new `FaceRecognizer128` class and points to `known_faces_128d_3.npz`
by default, so you can test the 128d approach without changing any existing files.
"""
...
# --- DEGISIKLIK: Orjinal FaceRecognizer yerine yeni FaceRecognizer128'i kullaniyoruz ---
from .face_recognizer_128d import FaceRecognizer128 as FaceRecognizer
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Live face recognition (128D Test)")
    ...
    # --- DEGISIKLIK: Varsayilan DB yolunu yeni 128d DB'ye cevirdik ---
    p.add_argument("--db-path", default="known_faces_128d_3.npz")
    p.add_argument(
        "--video",
        default=None,
        help="Optional video file path",
    )
    return p
...
def _run_mac(
    ...
    print("128D Sistem Aktif! Penceriyi kapatmak icin 'q' tusuna basin.\n")
    ...
    try:
        while running:
            ...
            now = time.time()
            if now - fps_t0 >= 1.0:
                print(f"Anlik FPS (128D): {fps_n / (now - fps_t0):.2f}")
                fps_t0 = now
                fps_n = 0

            cv2.imshow("Mac Live (128D Test)", frame)
...
def _run_rpi(
    ...
    print("128D Sistem Aktif! (Durdurmak icin terminalde CTRL+C)\n")
    ...
    try:
        while running:
            ...
            for det in last_dets:
                ...
                if name != "Unknown":
                    print(f"[BASARILI-128D] {name} tespit edildi! (Skor: {score:.3f})")
...
def _run_video(
    ...
    print(f"128D Video modu aktif: {video_path}")
    ...
    try:
        while True:
            ...
            cv2.imshow("Video (128D Test)", frame)
            if cv2.waitKey(1) & 0xFF == ord("q"):
                break
