...
import benchmark_video # calculate_iou ve calculate_ap icin kullanacagiz
...
def run_benchmark_for_size(video_path, teacher_model, student_model, main_size, recognizer, db_embs, db_names):
    ...
    while True:
        ...
        # Kameranin donanimsal olarak yaptigi daraltmayi (main_size) simule ediyoruz.
        # Video eger raw (1640x1232) formatindaysa, bu islem ISP downscaling'i taklit eder.
        if (orig_w, orig_h) != main_size:
            frame_resized = cv2.resize(frame, main_size)
        else:
            frame_resized = frame

        # 1. Teacher (Referans) - Ground truth'u kucultulmus kare uzerinden belirliyoruz
...
def main():
    parser = argparse.ArgumentParser(description="Ayni video uzerinde farkli main_size (kamera cozunurlugu) senaryolarini simule eder.")
    parser.add_argument("--video", required=True, help="Test edilecek video dosyasi yolu (ornek: test-videos/ornek.h264)")
    ...
    print("Yuz tanima modeli (buffalo_s) ve veritabani yukleniyor...")
    ...
    if db_path.exists():
        ...
        print(f"UYARI: Veritabani bulunamadi -> {db_path}. 'Unknown' donecektir.")
        db_embs, db_names = np.array([]), np.array([])

    # RPi'nin donanimsal olarak dondurebilecegi farkli main_size senaryolari
    main_sizes_to_test = [
        (112, 112),
        (160, 160),
        (320, 320),
    ...
    for main_size in main_sizes_to_test:
        print(f"\n--- Main Size: {main_size} Icin Test Basliyor ---")
        
        try:
            metrics = run_benchmark_for_size(video_path, teacher_model, student_model, main_size, recognizer, db_embs, db_names)
            
            if metrics:
                print(f"  Toplam Gercek Yuz: {metrics['total_truths']}")
                print(f"  Precision:         % {metrics['precision'] * 100:.2f}")
                print(f"  Recall:            % {metrics['recall'] * 100:.2f}")
                print(f"  F1-Score:          % {metrics['f1_score'] * 100:.2f}")
                print(f"  mAP@0.5:           % {metrics['ap_05'] * 100:.2f}")
                print(f"  Student Ort. FPS:  {metrics['fps_student']:.1f} FPS")
                
                print("  --- Tanima (Recognition) Istatistikleri ---")
                if not metrics["recognition_stats"]:
                    print("    Hicbir yuz taninmadi/cikartilmadi.")
                else:
                    for name, count in metrics["recognition_stats"].items():
                        print(f"    {name}: {count} kez")
            else:
                print("  Videoda yuz bulunamadi veya benchmark basarisiz oldu.")
