"""
Canli yuz tespiti + tanima — Raspberry Pi 5 icin optimize edilmis tek ONNX (YOLO INT8) + InsightFace.
...
YOLO_IMG_SIZE, export ile sabit giris boyuna esit olmalidir (ORT: Expected N Got M).
WiderFace modeli: 640x640 cikis; kucuk/uzak yuzler icin 640 sabit tutulur.
...
if not os.path.isfile(MODEL_PATH):
    if os.path.isfile(_int8_only):
        raise SystemExit(
            ...
            "INT8 ONNX yolunu MODEL_PATH ile eslestirin veya OPTIMIZED dosyasini ekleyin.\n"
            "Alternatif: .pt ile export: yolo.export(format='onnx', imgsz=640, int8=True, ...)"
        )
...
# Raspberry Pi OS: V4L2 backend ile dusuk gecikme
cap = cv2.VideoCapture(CAM_INDEX, cv2.CAP_V4L2)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
if not cap.isOpened():
    raise SystemExit("Webcam baslatilamadi (CAP_V4L2).")
...
while running:
    ...
    cv2.imshow("Face live (RPi5 INT8 ONNX)", frame)
