...
def start_rpi_preview(picam, width: int = 1280, height: int = 720) -> str | None:
    ...
    try:
        ...
        print("[UI] Picamera2 Preview module not found.")
        _rpi_preview_mode = None
        return None

    # SSH sessions do not have X11; DRM directly uses HDMI framebuffer.
    ...
    for preview_cls, name in backends:
        try:
            ...
            print(
                f"[UI] RPi preview started ({name}) @ {width}x{height}. "
                "Preview opens on the HDMI monitor connected to the Pi (not inside SSH console)."
            )
            return name
        except Exception as exc:
            print(f"[UI] {name} preview failed: {exc}")

    _rpi_preview_mode = None
    print(
        "[UI] WARNING: HDMI preview could not be opened. "
        "Is a monitor connected to HDMI? (Running via SSH is fine; "
        "the preview opens on the Pi's connected screen.)"
    )
    return None
...
def init_display(picam=None, window_title: str = WINDOW_TITLE, width: int = 1280, height: int = 720) -> str:
    ...
    if _probe_opencv_display(window_title):
        ...
        print("[UI] OpenCV preview active.")
        return _display_mode

    _display_mode = "headless"
    if not _gui_warned:
        print(
            "[UI] No display detected; running in headless mode "
            "(face recognition remains active, no preview — press CTRL+C to exit)."
        )
        _gui_warned = True
    return _display_mode
...
def _try_opencv_window(frame: np.ndarray, window_title: str) -> bool:
    ...
    try:
        ...
        if not _gui_warned:
            print(
                f"[UI] OpenCV window closed ({exc}); switching to headless mode."
            )
            _gui_warned = True
        return True
...
def show_frame(
    ...
    if _display_mode == "rpi" and picam is not None:
        try:
            ...
            _display_mode = "headless"
            if not _gui_warned:
                print(f"[UI] RPi overlay error; switching to headless mode: {exc}")
                _gui_warned = True
            return True
