...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        "If a relative name is provided, it will be resolved under ./test_videos/ when possible.",
    )
    return p


def resolve_model_path(project_root: Path, model_path: str) -> str:
    """Resolve YOLO model path, preferring `./yolo11-models/` for relative names."""

    p = Path(model_path)
    if p.is_absolute() and p.is_file():
        return str(p)

    # If the caller passes a bare filename, try yolo11-models/<name>
    candidate = project_root / "yolo11-models" / model_path
...
def resolve_video_path(project_root: Path, video_path: str) -> str:
    """Resolve video path, preferring `./test_videos/` for relative names."""

    p = Path(video_path)
    if p.is_absolute() and p.is_file():
        return str(p)

    candidate = project_root / "test_videos" / video_path
    if candidate.is_file():
        return str(candidate)
