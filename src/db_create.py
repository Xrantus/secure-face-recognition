...
Walks through `db_images/<PersonName>/*` images, detects a face, extracts an embedding,
...
def read_image_unicode(path: str) -> np.ndarray | None:
    """Read an image with Unicode characters in the file path."""
...
def create_db(
    ...
    if not person_dirs:
        raise SystemExit(
            f"ERROR: No personnel directories found! Example structure: {image_folder}/Ali/photo1.jpg"
        )

    global_embeddings: list[np.ndarray] = []
    global_names: list[str] = []

    print(f"Total of {len(person_dirs)} personnel directories found.")
    print("=" * 50)
    ...
    for person_dir in person_dirs:
        ...
        for filename in sorted(os.listdir(person_dir)):
            ...
            if img is None:
                print(f"  [!] {filename} could not be read or is corrupted.")
                continue

            dets = detector.detect(img)
            best = FaceDetector.best_by_conf(dets)
            if best is None:
                print(f"  [-] {filename}: face not found.")
                continue

            roi = crop_with_padding(img, best.bbox, config.MODEL_CONFIG.landmark_pad)
            if roi is None:
                print(f"  [-] {filename}: invalid ROI.")
                continue

            emb = recognizer.embed_from_roi(roi)
            if emb is None:
                print(f"  [-] {filename}: landmarks/embedding not found.")
                continue
        ...
        if per_embeddings:
            ...
            print(f"  => '{name}': processed {len(per_embeddings)} images, saved average embedding.")
        else:
            print(f"  => '{name}': No valid faces found, skipping.")

    print("\n" + "=" * 50)
    if not global_embeddings:
        print("RESULT: No embeddings could be saved.")
        return

    np.savez(output_npz_path, encodings=np.array(global_embeddings), names=np.array(global_names))
    print(f"Database saved to: '{output_npz_path}'")
    print(f"Total registered personnel: {len(global_names)}")
...
def main() -> None:
    ...
    image_folder = str(project_root / "db_images")
